...
!  rau0        = 1026._wp
  ...
  FUNCTION sigmantr( ptem, psal, kpi,kpj) 
    ...
    !! --------------------------------------------------------------------
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SCHEDULE(RUNTIME) MAP(from: sigmantr) MAP(to: ptem, psal )
    DO jj = 1, kpj
       DO ji = 1, kpi
          dl_t = ptem(ji,jj)
          dl_s = psal(ji,jj)
  ...
  FUNCTION spice_80 ( ptem, psal, kpi, kpj )
    ...
    dl_tempt(:,:)  = 1.d0
!$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SCHEDULE(RUNTIME) MAP(tofrom: dl_salt, dl_spi, dl_tempt) MAP(to: ptem, dl_salref)
    DO jjg=1, kpj
    DO ji=1,6
       dl_salt(:,jjg) = 1.d0
       DO jj=1,5
...
!                  sigmai_dep(ji,jj) = (  dlr * r1_rau0 - 1._wp  ) * dltm  ! density anomaly (masked)
  ...
  FUNCTION sigmai_dep_80 ( ptem, psal, pref, kpi,kpj,ld_eos80)
    ...
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SCHEDULE(RUNTIME) MAP(from: sigmai_dep_80) MAP(to: ptem, psal, dpr4, dlrs, dlref, dprau0)
    DO jj=1,kpj
       DO ji=1,kpi
...
!                 sigmai_dep2d(ji,jj) = (  dlr * r1_rau0 - 1._wp  ) * dltm  ! density anomaly (masked)
  ...
  FUNCTION sigmai_dep2d_80 ( ptem, psal, pref, kpi,kpj, ld_eos80)
    ...
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SCHEDULE(RUNTIME) MAP(from: sigmai_dep2d_80) MAP(to: ptem, psal, pref, dlrs)
    DO jj=1,kpj
       DO ji=1,kpi
  ...
  FUNCTION eosbn2_pol ( ptem, psal, pdep, pe3w, kpi, kpj, kup, kdown)
    ...
    dlh = pdep
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SCHEDULE(RUNTIME) MAP(from: eosbn2_pol) MAP(to: dlgrav, pe3w, r1_rau0, ptem, psal, dlh)
    DO jj = 1, kpj
       DO ji = 1, kpi
  ...
  FUNCTION eosbn2_80 ( ptem, psal, pdep, pe3w, kpi, kpj, kup, kdown, ld_eos80)
    ...
    zh = pdep
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SCHEDULE(RUNTIME) MAP(from: eosbn2_80) MAP(to: zgrav, pe3w, ptem, psal, zh)
    DO jj = 1, kpj
       DO ji = 1, kpi
  ...
  FUNCTION albet_2d(  ptem, psal, pdep, kpi, kpj)
    ...
    !!----------------------------------------------------------------------
    zh = pdep
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SCHEDULE(RUNTIME) MAP(from: albet_2d)  MAP(to: ptem, psal, zh)
     DO jj=1,kpj
       DO ji=1,kpi
  ...
  FUNCTION albet_z(  ptem, psal, pdep, kpk)
    ...
    REAL(KIND=8)    :: zt, zs, zh  ! working local variables
    !!----------------------------------------------------------------------
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SCHEDULE(RUNTIME) MAP(from: albet_z) MAP(to: pdep, ptem, psal, zh)
     DO jk=1,kpk
          zh = pdep(jk)
  ...
  FUNCTION beta_2d_80 (  ptem, psal, pdep, kpi, kpj, ld_eos80)
    ...
    !!----------------------------------------------------------------------
    zh = pdep
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SCHEDULE(RUNTIME) MAP(from: beta_2d_80) MAP(to: ptem, psal, zh)
    DO jj=1,kpj
       DO ji=1,kpi
          zt =  ptem(ji,jj)         ! potential temperature
          zs =  psal(ji,jj)- 35.0   ! salinity anomaly (s-35)
  ...
  FUNCTION beta_z_80 (  ptem, psal, pdep, kpk, ld_eos80)
    ...
    REAL(KIND=8)    :: zt, zs, zh  ! working variables
    !!----------------------------------------------------------------------
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SCHEDULE(RUNTIME) MAP(from: beta_z_80) MAP(to: pdep, ptem, psal, zh)
    DO jk=1,kpk
          zh = pdep(jk)
          zt =  ptem(jk)         ! potential temperature
          zs =  psal(jk)- 35.0   ! salinity anomaly (s-35)
